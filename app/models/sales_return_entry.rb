class SalesReturnEntry < ActiveRecord::Base
  ...
  def quantity_must_not_exceed_max_quantity
    ...
    delivery_entry = self.delivery_entry 
    max_return =  delivery_entry.quantity_sent - delivery_entry.quantity_lost 
  ...
  def entry_uniqueness
    ...
    elsif self.persisted? and not self.delivery_entry_id_changed? and sales_return_entry_count > 1
      errors.add(:delivery_entry_id , msg ) 
    elsif self.persisted? and self.delivery_entry_id_changed? and sales_return_entry_count != 0 
      errors.add(:delivery_entry_id , msg ) 
    end
  end
  ...
  def self.create_by_employee( employee, sales_return, params ) 
    return nil if employee.nil?
    return nil if sales_return.nil? 
