Ext.define('AM.controller.DeliveryLostEntries', {
  extend: 'Ext.app.Controller',

  stores: ['DeliveryLostEntries', 'DeliveryLosts'],
  models: ['DeliveryLostEntry'],

  views: [
    'sales.deliverylostentry.List',
    'sales.deliverylostentry.Form',
  ...
  init: function() {
    this.control({
      ...
      'deliverylostentrylist button[action=addObject]': {
        click: this.addObject
      },
      'deliverylostentrylist button[action=editObject]': {
        click: this.editObject
      },
      'deliverylostentrylist button[action=deleteObject]': {
        click: this.deleteObject
      },

			// monitor parent(delivery_lost) update
			'deliverylostlist' : {
	...
	reloadStore : function(record){
		...
		var store = this.getDeliveryLostEntriesStore();
  ...
  addObject: function() {
    ...
    var view = Ext.widget('deliverylostentryform', {
			parentRecord : record 
		});
  ...
  updateObject: function(button) {
    ...
    var values = form.getValues();
		...
		if( record ){
			...
			record.save({
				params : {
					delivery_lost_id : parentRecord.get('id')
				},
			...
			var me  = this; 
			var newObject = new AM.model.DeliveryLostEntry( values ) ;
    ...
    var record = this.getList().getSelectedObject();
		if(!record){return;}
		var parent_id = record.get('delivery_lost_id');
		var list  = this.getList();
