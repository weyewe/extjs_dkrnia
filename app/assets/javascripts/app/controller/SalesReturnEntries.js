Ext.define('AM.controller.SalesReturnEntries', {
  extend: 'Ext.app.Controller',

  stores: ['SalesReturnEntries', 'SalesReturns'],
  models: ['SalesReturnEntry'],

  views: [
    'sales.salesreturnentry.List',
    'sales.salesreturnentry.Form',
  ...
  init: function() {
    this.control({
      'salesreturnentrylist': {
        itemdblclick: this.editObject,
        selectionchange: this.selectionChange 
      },
      'salesreturnentryform button[action=save]': {
        click: this.updateObject
      },
      'salesreturnentrylist button[action=addObject]': {
        click: this.addObject
      },
      'salesreturnentrylist button[action=editObject]': {
        click: this.editObject
      },
      'salesreturnentrylist button[action=deleteObject]': {
        click: this.deleteObject
      },

			// monitor parent(sales_return) update
			'salesreturnlist' : {
	...
	reloadStore : function(record){
		...
		var store = this.getSalesReturnEntriesStore();
  ...
  addObject: function() {
    ...
    var view = Ext.widget('salesreturnentryform', {
			parentRecord : record 
		});
  ...
  updateObject: function(button) {
    ...
    var values = form.getValues();
		...
		if( record ){
			...
			record.save({
				params : {
					sales_return_id : parentRecord.get('id')
				},
			...
			var me  = this; 
			var newObject = new AM.model.SalesReturnEntry( values ) ;
    ...
    var record = this.getList().getSelectedObject();
		if(!record){return;}
		var parent_id = record.get('sales_return_id');
		var list  = this.getList();
